export class HttpRequestUtils {
  public static addXMLHttpRequestOverride(
    override: (xmlRequest: XMLHttpRequest, methode: string, url: string) => void,
  ): void {
  ...
  public static onSuccessfulGetRequest(callBack: (url: string, responseData: any) => void): void {
    const xmlRequestOverride = (xmlRequest: XMLHttpRequest, method: string, url: string) => {
      ...
      if (isSuccessfulGetRequest) {
        const responseData = HttpRequestUtils.tryParseToJson(xmlRequest.responseText);
  ...
  public static onSuccessfulDeleteRequest(
    callBack: (url: string, responseData: any) => void,
  ): void {
    const xmlRequestOverride = (xmlRequest: XMLHttpRequest, method: string, url: string) => {
      const isSuccessfulGetRequest =
        xmlRequest.readyState === XMLHttpRequest.DONE &&
