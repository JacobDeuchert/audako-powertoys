import { debounceTime, finalize, type Observable, Subject } from 'rxjs';
...
export class DomUtils {
  public static watchForDomChanges(
    element: HTMLElement | Document,
    debounce: number = 100,
  ): Observable<DomMutation> {
  ...
  public static mapHTMLCollection<T>(
    htmlCollection: HTMLCollection,
    modFunction: (element: Element, index?: number) => T,
  ): T[] {
    const result: T[] = [];
