...
} from './services/heal-check.service';
...
const findMatchingSystem = (
  url: string | undefined,
  systems: SystemSettings[] = [],
): SystemSettings | undefined => systems.find(system => url?.includes(system.url));
...
const createStorageChangeHandler =
  (
    onRegisteredSystemsChanged: (
      newSystems: SystemSettings[],
      oldSystems: SystemSettings[],
    ) => Promise<void>,
  ) =>
...
const createAlarmHandler =
  (
    deps: Pick<
      ServiceWorkerDeps,
      'getRegisteredSystems' | 'checkSystemHealth' | 'checkRichHealthCheck' | 'clearAlarm'
    >,
  ) =>
...
const registerListeners = (handlers: {
  ...
  onStorageChanged: (changes: {
    [p: string]: { newValue: any; oldValue: any };
  }) => Promise<boolean>;
...
const initializeHealthChecks = async (
  deps: Pick<ServiceWorkerDeps, 'observeSystemsHealth' | 'getRegisteredSystems'>,
) => {
  const registeredSystems = await deps.getRegisteredSystems();
