...
async function replaceMainScript(originalSrc: string): Promise<void> {
  try {
    ...
    if (!response.ok) {
      console.error('[audako-powertoys] Failed to fetch main script:', response.status, response.statusText);
      return;
    }
    ...
    console.info('[audako-powertoys] Modified script content:\n', scriptContent.replace(/;/g, ';\n'));
...
function modifyMainScript(content: string): string {
  ...
  const consoleSuppressMatch = content.match(/ci\.production\s*&&\s*\(window\.console\.log\s*=\s*function\s*\(\)\s*\{\s*\}\s*\)\s*;?/g);
  console.info('[audako-powertoys] Console suppression pattern found:', consoleSuppressMatch ? consoleSuppressMatch.length + ' match(es)' : 'none');
  content = content.replace(/ci\.production\s*&&\s*\(window\.console\.log\s*=\s*function\s*\(\)\s*\{\s*\}\s*\)\s*;?/g, '');

  // Intercept the Angular bootstrap to capture the root injector.
  const bootstrapMatch = content.match(/\.bootstrapModule\(([^)]+)\)\.catch\(/g);
  console.info('[audako-powertoys] Bootstrap pattern found:', bootstrapMatch ? bootstrapMatch.length + ' match(es): ' + JSON.stringify(bootstrapMatch) : 'none');
  content = content.replace(
    ...
    `.bootstrapModule($1).then(function(ref) {` +
      ...
      //   CONTEXT (component instance): always index 8
      ...
      // Expose helper to resolve any service by class/token
      `window.__ngGet__ = function(token) { return ref.injector.get(token); };` +

      // getLView: resolve __ngContext__ to LView (handles both numeric ID and direct LView)
      `window.__ngGetLView__ = function(el) {` +
      ...
      // getComponent: get the component instance from a DOM element
      `window.__ngGetComponent__ = function(el) {` +
        `var lView = window.__ngGetLView__(el);` +
      ...
      `console.log("[audako-powertoys] Helpers: __ngGet__(token), __ngGetLView__(el), __ngGetComponent__(el)");` +
      `return ref;` +
    `}).catch(`
  );

  // Log the rewritten bootstrap line for verification
  const rewrittenBootstrap = content.match(/\.bootstrapModule\([^)]+\)\.then\(function[\s\S]{0,200}/);
  console.info('[audako-powertoys] Rewritten bootstrap:', rewrittenBootstrap ? rewrittenBootstrap[0] : 'not found');
